...
t.test(p_ast~pres_burn, data=df1)
...
#two sided t-test
df1<-df %>% filter(year=="2019")
t.test(cp_07~pres_burn, data=df1)
```
...
#ASSOCIATION B/W PM10 and PED. ASTHMA
dat.m2<- df %>% subset(pres_burn=="1" & year=="2016")

m2<-glm.nb(p_ast ~ cp_07+TMAX+month +offset(log(popln)),data = dat.m2)
(est <- cbind(Estimate = coef(m2), confint(m2)))
exp(est)

#other
df.burn<- df %>% subset(pres_burn=="1" & year=="2016")
